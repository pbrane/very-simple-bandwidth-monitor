...
```bash
nohup ./bandwidth-monitor.sh > logs/bandwidth-monitor.out 2>&1 &
```

**Check status:**

```bash
tail logs/bandwidth-monitor.out
```

**View results:**

```bash
cat logs/bandwidth-log.csv
```
...
All output files are written to the `logs/` directory, which is gitignored.
